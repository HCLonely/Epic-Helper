  ...
  function addButton() {
    for (const el of $('a[href^="https://www.epicgames.com/store/"]:not(".Epic-add2lib")')) {
      ...
      if (/^https?:\/\/www\.epicgames\.com\/store\/.*?\/p\/.*/.test(href)) {
        $this.after(`<a class="add-to-library" href="javascript:void(0)" onclick="addToEpicLibrary(this)" data-href="${href}" data-type="store" target="_self">入库</a>`)
      }
      if (/^https?:\/\/www\.epicgames\.com\/store\/purchase.*/.test(href) && href.includes('namespace') && href.includes('offers')) {
        $this.after(`<a class="add-to-library" href="javascript:void(0)" onclick="addToEpicLibrary(this)" data-href="${href}" data-type="purchase" target="_self">入库</a>`)
      }
    }
  }
    ...
    */
    ...
    const url = await TM_request({
      ...
      .then(response => {
        ...
        const offers = response.responseText.match(/data-popover="pdp-(.*?)"/)?.[1]
        const namespace = response.responseText.match(/https:\/\/cdn1\.epicgames\.com\/(.*?)\/offer\//)?.[1]
        if (!pageId) {
          console.error(response)
          return null
        }
