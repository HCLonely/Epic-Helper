...
(function () {
  ...
  function requestOrderPage(namespace, offer) {
  ...
  function orderPreview(namespace, offer) {
    const url = `https://www.epicgames.com/store/purchase?namespace=${namespace}&offers=${offer}`
    const { XQW, dfpSessionId } = await requestOrderPage(namespace, offer)
    Swal.update({
      title: '正在获取syncToken...',
      text: url,
      icon: 'info'
      ...
      .then(response => {
        if (response.status === 200 && response.response) {
          if (!response.response.message && response.response.syncToken) {
            ...
            return { XQW, dfpSessionId, syncToken: response.response.syncToken }
  ...
  async function getGame({ title, id }, namespace) {
    const { XQW, dfpSessionId, syncToken } = await orderPreview(namespace, id)
    Swal.update({
      title: '正在领取...',
      text: id,
      icon: 'info'
  ...
  unsafeWindow.addToEpicLibrary = async function (el) {
    ...
    */
    ...
    const freeOffers = await getDiscountInfo(href)
    if (!freeOffers) {
      return null
